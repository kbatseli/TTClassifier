...
while  ite < itemax  
    ...
    vn=kron(reshape(Matn{ind+1}, r(ind+1), r(ind+1)), reshape(Matn{ind}, r(ind), r(ind)));
    vn=vn';
    D=kron(eye(n(ind)+1), vn);
    D=reshape(D, r(ind), r(ind+1), n(ind)+1, r(ind), r(ind+1), n(ind)+1);
    D=permute(D, [1 3 2 4 6 5]);
    D=reshape(D, r(ind)*(n(ind)+1)*r(ind+1), r(ind)*(n(ind)+1)*r(ind+1));   
    
    %gamma=1;
    y=pinv(C'*C + gamma*D)*(C'*b);
    
    res(ite)=norm(C*y-b)^2/2 + gamma/2*(y'*D*y);    % Regularized LS loss function
